...
class Dataset:
    def __init__(self):
        ...
        self.X = (self.X - ((np.amax(self.X) + np.amin(self.X)) / 2) ) / ((np.amax(self.X) - np.amin(self.X)) / 2)

        self.Y = np.array(self.Y)
        self.Y = (self.Y - ((np.amax(self.Y) + np.amin(self.Y)) / 2)) / ((np.amax(self.Y) - np.amin(self.Y)) / 2)
        #TODO normalize data
